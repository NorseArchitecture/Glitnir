...
## Amendment (2026-08-09) — facade controllers live in the owning realm; the ban is wire-format machinery, not residence

**Operator verdict, session 2026-08-09**, rendered while placing Mímir's first facade controller. §3's "facade controllers are host-compilation source, always" is overturned. Declaring platform surface in Yggdrasil was only ever a proving-ground allowance — nothing is *declared* there unless it is being tested ahead of moving to its proper home — so a realm's facade controllers are realm source: they live beside the service they front, in `{Realm}.Web.Server`, deriving Asgard's `GrpcControllerBase`. What a realm-resident controller may never do is pull wire-format machinery — System.Text.Json, Json.NET, protobuf specifics — into the realm assembly; that is NORSE070's standing jurisdiction and was always the substance of this rule. Residence was the accident.

Mechanical consequences:

- **The XML shape generator's discovery widens** from a syntax predicate over the running compilation to a symbol walk over the host's reference closure: `GrpcControllerBase` descendants in referenced realm assemblies are discovered by metadata, their action signatures read symbolically, and the shapes still emit into the host compilation — beside the formatters, name tables, and registry that consume them, so nothing about §5's mechanism moves. The 2026-08-02 ratification's "structurally blind" premise was an implementation fact awaiting a fix, not a law.
- **The `AddNorseXml` startup tripwire (§3) is unchanged** and remains the loud guard for any discovery gap — including the interim window before the walker widens, during which a realm-resident facade fails host startup with the named error rather than a silent runtime 500.
- **Exposure curation survives at the composition root as it always did:** the host chooses which realm server assemblies it references and maps. Per-action curation moves with the controller to the realm that owns the service — the realm declares its own ambassador desk.

**First consumer:** Mímir's `CountriesController` (`Reference.Web.Server`), authored the same day.
