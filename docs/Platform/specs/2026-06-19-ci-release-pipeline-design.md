...
**`workflow_call` permission chain — confirmed 2026-06-27.** GitHub enforces declared permissions at every level of a `workflow_call` stack at parse time, regardless of whether the guarded step actually runs on the event. If a reusable workflow's job declares `pull-requests: write` (e.g., `ci-build-test.yml`'s `build` job, for the coverage comment), then every caller in the chain — `release-container.yml` and the thin `release.yml` — must also grant `pull-requests: write`, or the workflow is rejected with a validation error. The rule: grant all permissions that any descendant job declares, at every level of the caller chain.
