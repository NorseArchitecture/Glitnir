...
**Amendment (2026-07-25):** live during Task 13/14 (Yggdrasil), the `NorseGatewayEmissionMode` MSBuild property this mechanism depends on was found never declared `CompilerVisibleProperty` platform-wide — only worked around locally in two Yggdrasil `.csproj` files. Without it, the generator can't see the property via `AnalyzerConfigOptions`, and `InProcessHost` mode silently falls back to `Contract` mode — no error, no diagnostic. Scoped for a near-future session, not yet fixed at the source (Asgard's generator packaging). See `../plans/2026-07-24-transport-neutral-invocation-pipeline.md` Task 13 Step 5.
...
**Amendment (2026-07-25):** the four standard `Behavior` classes this section describes (`TelemetryBehavior`, `ExceptionTranslationBehavior`, `AuthorizationBehavior`, `ValidationBehavior`) are `internal` by the platform's `omit_if_default` convention (plan Tasks 3-4), but the generated in-process gateway constructs them by name in a different assembly (plan Task 9). That needs a per-consumer `InternalsVisibleTo` grant from Midgard, which this design doesn't call out. Found live, fixed and staged in Midgard — not yet shipped to NuGet. See `../plans/2026-07-24-transport-neutral-invocation-pipeline.md` Task 9.
