...
**Status:** DRAFT for review (brainstorm output, 2026-08-04). No plan, no implementation, until this passes the gate.
...
## 1. Rulings absorbed from the brainstorm (2026-08-04)
...
`NorseNpgsqlMigrationsSqlGenerator` derives from Npgsql's generator, registered via `ReplaceService<IMigrationsSqlGenerator, …>` inside `NorsePostgresEfProvider.Configure` — the existing single choreography point. An annotation-provider companion surfaces `Norse:Temporal` onto migration operations so the generator sees it on creates, drops, and column operations (exact EF service seam — `IRelationalAnnotationProvider` vs consulting the target model in `Generate` — is a plan-time verify item; both are plumbing under the same design).
...
1. `CREATE EXTENSION IF NOT EXISTS btree_gist` — hard prerequisite for `WITHOUT OVERLAPS` (POC-confirmed; first beta1 run failed without it).
2. `system_period tstzrange NOT NULL DEFAULT tstzrange(now(), 'infinity')` on the main table.
...
4. Trigger function (`SECURITY DEFINER`, owned by the migration role) plus UPDATE/DELETE triggers on the main table: UPDATE closes the old version into history (`tstzrange(lower(OLD.system_period), now())`) and resets `NEW.system_period` to `tstzrange(now(), 'infinity')`; DELETE inserts the closed old version. Explicit column lists, never `SELECT *` or positional inserts.
...
### 3.2 Evolution — history mirrors main, forever

Column operations against a temporal table are the generator's job for the life of the schema:

- **`AddColumn`** → mirrored onto history **as nullable regardless of the main column's nullability or default** — history rows predating the column honestly say NULL. Trigger function regenerated with the new column list; view re-emitted via `CREATE OR REPLACE VIEW`.
...
### 3.3 Free rider: the DBA schema dump
...
5. **Bifröst proof:** clean `dotnet run --project src/Orchestration.AppHost` stands up `norse_identity` with the apparatus; the verification test writes through a temporal entity and asserts the history row is visible on **both the primary and the streaming replica**.
...
- **Role separation (Norns §10 grants matrix):** the `SECURITY DEFINER` shape ships now; the actual migration-role/runtime-role split and grant emission land with the grants story.
