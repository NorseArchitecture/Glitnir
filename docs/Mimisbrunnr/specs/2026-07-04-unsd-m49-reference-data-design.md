...
**Design rule this follows — the "many side" heuristic:** a denormalized ancestor-chain document belongs on the entity at the *many* side of a one-to-many hierarchy (each row has exactly one bounded upward path), never on the *one* side (which would have to embed an unbounded collection of descendants). `CountryOrArea` is the many side relative to `Region`, so `View` lives there. `Region` itself never gets a reciprocal "my descendant countries" JSON column — that direction doesn't bound. This is also, structurally, why a future generic "projection expression" helper over any entity's `View` would never need a special guard against expanding into a collection: a `View` graph can never contain one, by this rule.
