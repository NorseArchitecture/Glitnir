...
- **Correction found during whole-branch review (2026-07-09):** the list above omitted the fields that actually make the package *consumable* — `"files"`, `"main"`, `"exports"`, and a `"prepack"` script. Without them, npm falls back to `.gitignore` to decide what ships, which excludes `dist/` — the built output is the entire point of this package. Every future npm package on this platform must set: `"files": ["dist"]` (scope the tarball to build output only — `package.json`/`README.md`/`LICENSE` ship regardless), `"main"`/`"exports"` (point consumers at the built entry points, not source), and `"prepack": "npm run build"` (guarantee `dist/` is fresh and present at publish time, never stale or absent). This is the platform's first npm package and sets the template — this omission should not repeat.
