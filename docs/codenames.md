...
| **Naglfar** | `Norse.DesignSystem` | The ship built from dead men's nails, captained by giants, to ferry the end of the world. Assembled from the unglamorous remnants — tokens, radii, deprecated variants — into something seaworthy enough to carry every product UI into battle. Every design system eventually gets replaced; that's a feature, not a flaw — Naglfar doesn't survive Ragnarök, but it delivers everything else there first. npm-only, no .NET — `DesignSystem.Stories` split out to Bragi 2026-07-12, the same day it landed here. |
| **Bragi** | `Norse.DesignSystem.Stories` | The skaldic god of poetry, keeper of every tale worth telling — Bragi doesn't build the ship, he sings of everything aboard it. Content-only Razor Class Library of `.stories.razor` catalog pages for the platform's Blazor components; rides `NorseRef` on every realm that publishes one — Asgard's `Abstractions.Components` today, `AuthN.Components.FluentUI` (Heimdall) and `ReferenceData.Components.FluentUI` (Mimir) as each ships. Ships no runnable app of its own — Yggdrasil's `Hosting.Stories.Client`/`.Server` hosts the runnable BlazingStory catalog built from it, published to `ghcr.io/norsearchitecture/hosting/stories`. Named 2026-07-12, split from Naglfar the same day `DesignSystem.Stories` first landed there. |
