...
**Amended 2026-06-25:** This plan was written before the Asgard project structure was settled. The egress contracts are **not** a standalone `Norse.Abstractions.Egress` project. They land in `Norse.Abstractions.Backend` under the `Norse.Abstractions.Backend.Egress` namespace, with source files at `src/Abstractions.Backend/Egress/` and tests in `tests/Abstractions.Backend.Tests/`. The repo scaffold (Task 1) is also superseded — the solution structure, `Directory.Build.props` ladder, and `Asgard.slnx` are governed by the project structure design (`../specs/2026-06-25-asgard-project-structure-design.md`), which covers all six assemblies. Every type, test, and implementation step below is otherwise unchanged; substitute `Norse.Abstractions.Backend.Egress` for `Norse.Abstractions.Egress` throughout, and `Abstractions.Backend` for `Abstractions.Egress` in all project/file paths. The scaffold this plan's Task 1 was superseded by is complete (`2026-06-25-asgard-scaffold.md`); Tasks 2–6 below are ready to execute.
...
**Goal:** Implement the egress contracts slice of the platform egress spec in `Norse.Abstractions.Backend`: the `HttpResult<T>` discriminated union, the closed `EgressError`/`FailureKind` vocabulary, the `EgressClassifier` response-classification seam, and the `IHttpEgress`/`IResponseParser<T>` injection surfaces — all under the `Norse.Abstractions.Backend.Egress` namespace. Nothing past HttpClient mechanics and the union — no auth seam, no resilience-profile enum, no registration extensions (those are Infrastructure/Hosting work, future Midgard/Yggdrasil plans).

**Architecture:** A hand-authored, zero-boxing C# union (mirroring `Result<T>` in `Norse.Primitives` exactly: `[MustConsume]` + `[Union]`/`IUnion`, per-case constructors, `TryGetValue` overloads, `Match`) plus a delegate-based classification seam and two narrow interfaces. Asgard is a bare shell today — this plan's Task 1 covers only the egress project and test project within the repo scaffold defined in `../specs/2026-06-25-asgard-project-structure-design.md`.
