...
### 12. The serving realm reached into Midgard, and the data realm reached back up (2026-08-01)
- **Believed:** Mimir's `Reference.Web.Server` calling Midgard's `AddNorseWell<ReferenceDbContext>()` was legitimate composition — a serving realm wiring its own persistence — and the same era's generated-enum design (`IsoCountryCode` baked in Mimir, consumed by Mimisbrunnr's entities and seed contributor) was a reasonable division of labor. Both shipped in the well-and-wire slice and sat quietly; the same night's well-composition plan (Task 4) built further on the Midgard edge.
- **Wrong because:** two dependency inversions, one per direction. Midgard is composition-root material — it takes Æsir law and produces working code plus conventions Yggdrasil stitches together; a realm that references it has annexed the composition root's job (and Mimir had also annexed its provider decision, hardcoding Postgres). And Mimisbrunnr consuming Mimir's generated surface put the data realm downstream of its own serving layer — a full bidirectional cycle between the two reference realms, with the seed data stranded on the serving side.
- **Surfaced when:** real package-mode CI on Mimir PR #17 failed to resolve `AddNorseWell` — `UseProjectReferences=true` had hidden the edge locally since it shipped (the same dormant-until-package-mode failure shape as the self-consuming-generator gap one night earlier). Buvy's real-time verdict: "mimir cannot depend on midgard so full stop halt do not catch fire here." The cycle half surfaced in the next session's cleanup inventory.
- **Correction:** the dependency doctrine written down as law — Svartálfheim baked in everywhere, Asgard sliced by domain shape, chassis realms consumable by design, **Midgard consumed by Yggdrasil alone** (MSBuild-enforced), Yggdrasil stitching all of it IoC-style. The Midgard edge dies via the `[NorseWell]` declaration + seam generator (realms declare, the composition root's generator discovers and wires, the runtime DbContext is generated `file`-scoped — no realm authors one). The cycle dies via relocation: seeds, generator, and generated surface all move down to Mimisbrunnr (`Reference.Data.Primitives`/`.Namespaces`), and Mimir consumes them in the one legal direction. Specs: `Platform/specs/2026-08-01-well-seam-midgard-excision-design.md`, `Platform/specs/2026-08-01-reference-data-dependency-inversion-design.md`.
- **Lesson:** meta-lesson #5 again, at realm scale — where a type lives is a dependency claim, and where a *call* lives is one too. A composition call sitting in a realm is that realm claiming the composition root's job; generated vocabulary sitting in the serving layer is the serving layer claiming the data realm's charter. And the two-crossings split keeps paying for itself: velocity mode hides exactly this class of sin, so only a real package-mode gate renders the verdict.
