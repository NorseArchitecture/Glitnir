...
- **Entity Framework Core** with snake_case and `MaxLength` conventions. **The abstract, law-enforcing base context — audit stamping, convention enforcement — lives in `Norse.EntityFramework` (Urdarbrunnr; ruled 2026-06-11).** The concrete per-service DbContext family (`BillingDbContext`, `ClaimsDbContext`, …) is registered and pooled by `Norse.Infrastructure.Persistence`, which applies each `.Worker`'s `IEntityTypeConfiguration<T>` impls at startup; whether those concrete contexts are hand-declared, source-generated, or materialized by Infrastructure on the service's behalf is **open** — see `docs/superpowers/specs/2026-06-11-entityframework-context-provenance-decision-inputs.md`. Services never inject a DbContext or call `SaveChangesAsync`; SQL entities live solely in `.Worker` — the web tier cannot reach the system of record by construction.
