...
| **Midgard** | `Norse.Infrastructure` | platform | Embodied law — repository implementations (incl. temporal), `JsonControllerBase<TService>`, mediator runtime, UI Composition |
...
| **Ratatoskr** | `Norse.NServiceBus` | platform | The squirrel — NServiceBus endpoint configuration, saga infrastructure, message conventions, and transport wiring; Asgard declares the messaging surface, Ratatoskr carries it |
...
| **Himinbjorg** | `Norse.Identity` | platform | Heimdall's hall — EF persistence for ASP.NET Identity and OpenIddict: entities, conventions, and migrations; sealed server-side, never referenced from WASM or MAUI |
| **Heimdall** | `Norse.Access` | platform | The watchman — auth services riding on Himinbjorg's identity record: one access ruleset across Blazor Server, WASM, and MAUI, with admin components and the backing gRPC service |
| **Gjallarhorn** | `Norse.Observability` | platform | Cross-cutting telemetry, alerting; the system-wide signal |
| **Nagalfar** | `Norse.DesignSystem` | platform | Design tokens, radii, and component primitives — standalone for now, no declared consumers |
...
**Future services:** load-bearing, platform-wide, product-agnostic → its own `Norse.{Function}` namespace; product-domain-specific → `{Company}.{Function}`. Identity + access (split: `Norse.Identity` / `Norse.Access`), observability, and AI are platform; **fraud and triage (lore: Tyr, Valkyrie) are unplaced** — lifeblood in insurance, minor elsewhere, so platform-vs-product is unsettled (in the ether — `docs/codenames.md`). The dictionary records placement case-by-case.
...
Norse mythology only. **Do not mix pantheons.** **Codenames are lore** — the genesis/inspiration story; code and specs use the **function** (`Norse.{Function}` substrate / `{Company}.{Context}.*` products — §5). The full **ethos⇒function dictionary** (codename → namespace + rationale) is `docs/codenames.md`; the brand synthesis is `docs/norse-architecture.md`. **Platform realms** (named for function; lore in parens): Abstractions (Asgard) · Infrastructure (Midgard) · Primitives (Svartalfheim) · EntityFramework (Urdarbrunnr) · NServiceBus (Ratatoskr) · Hosting (Yggdrasil) · Orchestration (Bifrost) · Warehouse (Muninn) · Identity (Himinbjorg) · Access (Heimdall) · Observability (Gjallarhorn) · AI (Mimir) · DesignSystem (Nagalfar) · Shell (Hlidskjalf, per-product). **Product realms** are named for the governing figure of their vertical (rule #3); the assignments are each venture's own record, not this corpus's. **In the ether** (unplaced): fraud (Tyr) and triage (Valkyrie) — insurance-lifeblood, platform-vs-product unsettled. The **Reserved-with-an-intended-use tier was killed 2026-06-07** (predictions rot — see `docs/the-crooked-path.md`); the **bench** is a no-meaning palette (Norns, Huginn, Saga, Bragi, Var, Idunn, Vidar) — a name leaves it only when a real component takes it. Glitnir is this repository (the design court).
